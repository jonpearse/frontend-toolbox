/*********************************************************************************************************************
 *
 * General FX library.
 *
 *********************************************************************************************************************/

/** -- INTERNAL FUNCTIONS -- */
const DEFAULT_SPEED = 400;

/**
 * Applies an effect to the specified element. This is a wrapper for CSS animation
 *
 * @param   {HTMLElement}   el          the element we’re effecting
 * @param   {string}        sFxName     the name of the effect to apply (in CSS: fx--#{sFxName} )
 * @param   {integer}       iSpeed      the speed of the animation, in ms
 * @return  {Promise} a promise that is resolved once everything is complete
 */
function _apply(el, sFxName, iSpeed)
{
    return new Promise(resolve =>
    {
        // 1. set an event handler (old-school!)
        function _complete(ev)
        {
            // if it’s not what we’re after…
            if (ev.target !== el)
            {
                return;
            }

            // tidy up
            el.removeEventListener('animationend', _complete);
            el.style.animationDuration = null;
            el.classList.remove(`fx--${sFxName}`);

            // resolve the promise
            resolve();
        }

        // 2. run everything
        el.addEventListener('animationend', _complete);
        if (iSpeed !== null)
        {
            el.style.animationDuration = iSpeed + 'ms';
        }
        el.classList.add(`fx--${sFxName}`);

    });
}

/**
 * Performs a slide animation where the element slides up to nothingness and disappears.
 *
 * @param   {HTMLElement} el - the unfortunate victim
 * @param   {Number} speed - the time (in ms) to run the effect over
 * @return  {Promise} a Promise that is resolved when the effect completes
 */
function slideUp(el, speed = DEFAULT_SPEED)
{
    // 1. set our max height and apply
    el.style.maxHeight = `${el.clientHeight}px`;

    return new Promise(resolve =>
    {
        _apply(el, 'slideUp', speed).then(() =>
        {
            // set hidden, tidy our maxHeight, and return
            el.style.display   = 'none';
            el.setAttribute('aria-hidden', 'true');
            el.style.maxHeight = null;

            resolve();
        });
    });
}

/**
 * Performs a slide animation where the element is revealed.
 *
 * @param   {HTMLElement} el - the unfortunate victim
 * @param   {Number} speed - the time (in ms) to run the effect over
 * @return  {Promise} a Promise that is resolved when the effect completes
 */
function slideDown(el, speed = DEFAULT_SPEED)
{
    // 1. unhide the element
    el.style.display = '';
    el.removeAttribute('aria-hidden');
    el.style.maxHeight = `${el.clientHeight}px`;

    // 1. apply the slide
    return new Promise(resolve =>
    {
        _apply(el, 'slideDown', speed).then(() =>
        {
            el.style.maxHeight = null;
            resolve();
        })
    });
}

/**
 * Fades the specified element in.
 *
 * @param   {HTMLElement} el - the unfortunate victim
 * @param   {Number} speed - the time (in ms) to run the effect over
 * @return  {Promise} a Promise that is resolved when the effect completes
 */
function fadeIn(el, speed = DEFAULT_SPEED)
{
    return new Promise(resolve => _apply(el, 'fadeIn', speed).then(resolve));
}

/**
 * Fades the specified element out.
 *
 * @param   {HTMLElement} el - the unfortunate victim
 * @param   {Number} speed - the time (in ms) to run the effect over
 * @return  {Promise} a Promise that is resolved when the effect completes
 */
function fadeOut(el, speed = DEFAULT_SPEED)
{
    return new Promise(resolve => _apply(el, 'fadeOut', speed).then(resolve));
}

/**
 * Kooky combination effect, where the element is faded out and then slid up. This uses a proxy element to work around
 * limitations in what animating max-height is capable of.
 *
 * @param   {HTMLElement} el - the unfortunate victim
 * @param   {Object} options - an object containing speed and split options
 * @return  {Promise} a Promise that is resolved when the effect completes
 */
function fadeUp(el, options = {})
{
    // 0. do things with the options
    options = Object.assign({}, { split: .6, speed: DEFAULT_SPEED }, options);

    // 1. work out how things are split
    let fFadeTime  = options.split * options.speed;
    let fSlideTime = options.speed - fFadeTime;

    // 2. start to fade things
    return new Promise(resolve =>
    {
        fadeOut(el, fFadeTime).then(() =>
        {
            // a. when it’s faded out, replace it with a DOM element of the same height
            let elTmp = document.createElement('div');
            elTmp.classList.add('fx__tmp');
            elTmp.style.height = `${el.clientHeight}px`;

            // b. switch things around
            el.parentNode.insertBefore(elTmp, el);
            el.style.display = 'none';
            el.setAttribute('aria-hidden', 'true');

            // c. slide the temp element away
            slideUp(elTmp, fSlideTime).then(() =>
            {
                // remove our errant node and callback
                elTmp.parentNode.removeChild(elTmp);
                resolve();
            });
        })
    });
}

/**
 * Reverse effect of fadeUp, above.
 *
 * @param   {HTMLElement} el - the unfortunate victim
 * @param   {Object} options - an optional object containing speed/proportion options
 * @return  {Promise} a Promise that is resolved when the effect completes
 */
function fadeDown(el, options)
{
    // 0. default options
    options = Object.assign({}, { split: .6, speed: DEFAULT_SPEED }, options);

    // 1. work out how things are split
    let fFadeTime  = options.split * options.speed;
    let fSlideTime = options.speed - fFadeTime;

    // 2. create a dummy node of the appropriate height…
    // a. show our victim, so we can read a height
    el.style.display = null;

    // b. create the new element
    let elTmp = document.createElement('div');
    elTmp.classList.add('fx__tmp');
    elTmp.style.height  = `${el.clientHeight}px`;

    // c. hide our victim, and insert the temporary element
    el.style.display = 'none';
    el.parentNode.insertBefore(elTmp, el);

    // d. trigger the slide animation
    return new Promise(resolve =>
    {
        slideDown(elTmp, fSlideTime).then(() =>
        {
            // a. remove our temporary element and show the real one
            elTmp.parentNode.removeChild(elTmp);
            el.style.display = null;

            // b. fade it in!
            fadeIn(el, fFadeTime).then(resolve);
        });
    })
}

/**
 * Stops any effects that may be running at the moment.
 *
 * @param {HTMLElement} el - the element on which to stop effects
 */
function stop(el)
{
    let sClass = el.className.replace(/\s*fx\-\-\w\s/g, '');
    el.className = sClass;
}

module.exports = {

    // slide only
    slideUp:   slideUp,
    slideDown: slideDown,
    slideToggle: (el, options) =>
    {
        // stop any existing effects
        stop(el);

        // work out what to run based on visibility
        if (window.getComputedStyle(el).display.toLowerCase() === 'none')
        {
            slideDown(el, options);
        }
        else
        {
            slideUp(el, options);
        }
    },

    // fade only
    fadeIn:    fadeIn,
    fadeOut:   fadeOut,

    // combination fade+slide
    fadeUp:    fadeUp,
    fadeDown:  fadeDown,

    // progress squidging
    openProgress:  (el, speed = null) => new Promise(resolve => _apply(el, 'openProgress', speed).then(resolve)),
    closeProgress: (el, speed = null) => new Promise(resolve => _apply(el, 'closeProgress', speed).then(resolve)),

    // stop
    stop: stop
};
